        ...
        [(1, 0), (-1, 0), (0, 1), (0, -1)].iter().for_each(|dir| {
            ...
            if p2.0 >= 0 && p2.0 < 11 && p2.1 >= 0 && p2.1 < 11 {
                ...
                if board.fields[p1.0][p1.1] != Empty && (BoardConfiguration::special_squares().contains(&p2) || board.fields[p2.0][p2.1] != Empty) {
                    //If the piece at p1 is enemy and not the king, and the piece at p2 is friendly
                    if board.fields[p1.0][p1.1].player().unwrap() != board.turn && board.fields[p1.0][p1.1] != WhiteKing &&
                        (BoardConfiguration::special_squares().contains(&p2) || board.fields[p2.0][p2.1].player().unwrap() == board.turn) {
                        //Capture p1
